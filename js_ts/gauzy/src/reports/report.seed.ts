...
import { environment as env, DatabaseTypeEnum } from'../../plugins/config/dist/index';
import { IReport, IReportCategory, IReportOrganization, ITenant } from '../../plugins/contracts/dist/index';
...
async function cleanReport(dataSource: DataSource, config: Partial<ApplicationPluginConfig>) {
    ...
    const dbType = config.dbConnectionOptions.type as DatabaseTypeEnum;
    ...
    try {
        ...
        await new Promise<void>((resolve, reject) => {
            rimraf(`${dir}/!(rimraf|.gitkeep)`, (err) => {
                if (err) {
                    console.error(chalk.red(`Error cleaning up report images: ${err.message}`));
                    reject(err);
                } else {
        ...
        console.error(chalk.red(`Error during cleanReport: ${error.message}`));
    }
}
