...
class CloudinaryProvider extends provider_1.Provider {
    instance;
    name = contracts_1.FileStorageProviderEnum.CLOUDINARY;
    config;
    ...
    getCloudinaryInstance() {
        try {
            ...
            console.error(`Error while retrieving ${contracts_1.FileStorageProviderEnum.CLOUDINARY} instance:`, error);
        }
    }
    /**
        ...
        try {
            ...
            if (request) {
                ...
                if (settings) {
                    ...
                    if ((0, common_1.trimAndGetValue)(settings.cloudinary_cloud_name))
                        this.config.cloud_name = (0, common_1.trimAndGetValue)(settings.cloudinary_cloud_name);
                    if ((0, common_1.trimAndGetValue)(settings.cloudinary_api_key))
                        this.config.api_key = (0, common_1.trimAndGetValue)(settings.cloudinary_api_key);
                    if ((0, common_1.trimAndGetValue)(settings.cloudinary_api_secret))
                        this.config.api_secret = (0, common_1.trimAndGetValue)(settings.cloudinary_api_secret);
                    if ((0, common_1.isNotEmpty)(settings.cloudinary_api_secure)) {
                    ...
                    if ((0, common_1.isNotEmpty)(settings.cloudinary_delivery_url))
                        this.config.baseUrl = new url_1.URL(settings.cloudinary_delivery_url).toString();
                }
            }
        }
        catch (error) {
        ...
        try {
            ...
            return new multer_storage_cloudinary_1.CloudinaryStorage({
                ...
                params: (_req, file) => {
                    ...
                    const folder = (0, path_1.join)(destination).replace(/\\/g, '/');
                    ...
                    if (filename) {
                        ...
                        public_id = `${prefix}-${(0, moment_1.default)().unix()}-${parseInt('' + Math.random() * 1000, 10)}`;
                    }
            ...
            console.error(`Error while creating ${contracts_1.FileStorageProviderEnum.CLOUDINARY} storage engine:`, error);
            return null;
        }
    }
    /**
    ...
    async url(fileURL) {
        ...
        return new url_1.URL((0, path_1.join)(this.config.cloud_name, fileURL), this.config.baseUrl).toString();
    }
    /**
    ...
    path(filePath) {
        ...
        try {
            ...
            return new url_1.URL((0, path_1.join)(this.config.cloud_name, filePath), this.config.baseUrl).toString();
        }
        catch (error) {
    ...
    async putFile(file, path = '') {
        return new Promise((resolve, reject) => {
            ...
            const public_id = (0, path_1.join)(path).replace(/\\/g, '/');
    ...
    async mapUploadedFile(file) {
        if ((0, common_1.isNotEmpty)(file.filename)) {
