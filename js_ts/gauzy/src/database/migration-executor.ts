...
import { ApplicationPluginConfig, isNotEmpty } from '../../plugins/common/dist/index';
 ...
 * @param config
 */
async function establishDatabaseConnection(config: Partial<ApplicationPluginConfig>): Promise<DataSource> {
    ...
    try {
        ...
        console.log(chalk.black.bgRed('Unable to connect to database'));
        console.error(error);
    }
    return dataSource;
}
...
export class ${camelCase(name, true)}${timestamp} implements MigrationInterface {
    ...
    */
    public async postgresUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
        ${isDatabaseType([DatabaseTypeEnum.postgres], connection.options as any) ? upSqls.join(`
        `) : [].join(`
        `)}
    }
    ...
    */
    public async postgresDownQueryRunner(queryRunner: QueryRunner): Promise<any> {
        ${isDatabaseType([DatabaseTypeEnum.postgres], connection.options as any) ? downSqls.join(`
        `) : [].join(`
        `)}
    }
    ...
    */
    public async sqliteUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
        ${(isSqliteDB(connection.options as any)) ? upSqls.join(`
        `) : [].join(`
        `)}
    }
    ...
    */
    public async sqliteDownQueryRunner(queryRunner: QueryRunner): Promise<any> {
        ${(isSqliteDB(connection.options as any)) ? downSqls.join(`
        `) : [].join(`
        `)}
    }
    ...
    public async mysqlUpQueryRunner(queryRunner: QueryRunner): Promise<any> {
        ${isDatabaseType([DatabaseTypeEnum.mysql], connection.options as any) ?
            upSqls.join(``) :
            [].join(``)
        }
    }
