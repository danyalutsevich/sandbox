...
const commands_1 = require("integration-map/commands");
const commands_2 = require("tasks/commands");
const commands_3 = require("tags/commands");
const github_repository_service_1 = require("./repository/github-repository.service");
const commands_4 = require("./repository/issue/commands");
let GithubSyncService = exports.GithubSyncService = class GithubSyncService {
    _commandBus;
    _octokitService;
    _integrationTenantService;
    _organizationProjectService;
    _githubRepositoryService;
    logger = new common_1.Logger('GithubSyncService');
        ...
        try {
            ...
            try {
                ...
                this.getRepositoryAllIssues(installation_id, owner, repo, (issues) => {
                    console.log(chalk_1.default.magenta(`Automatically syncing ${issues.length} issues`));
        ...
        try {
            ...
            try {
                        ...
                        case contracts_1.IntegrationEntity.ISSUE:
                            ...
                            if (!!issueSetting.sync) {
                                for await (const issue of issues) {
                                    console.log(chalk_1.default.green(`Processing Issue Sync: %s`), issue.id);
                                    ...
                                    await this._commandBus.execute(new commands_4.IntegrationSyncGithubRepositoryIssueCommand({
                                        tenantId,
                                        organizationId,
                                        integrationId
                                    }, repositoryId, issue));
                                    ...
                                    const integrationMap = await this._commandBus.execute(new commands_1.IntegrationMapSyncIssueCommand({
                                        entity: {
                                            title,
                                            description: body,
                                            status: state,
                ...
                await this._integrationTenantService.update(integrationId, {
                    lastSyncedAt: (0, moment_1.default)()
                });
    ...
    async syncGithubLabelsByIssueNumber({ organizationId, tenantId, integrationId, repository, issue }) {
        try {
            ...
            if (settings && settings.installation_id) {
                ...
                const labelsToCheck = [contracts_1.SYNC_TAG_GITHUB, contracts_1.SYNC_TAG_GAUZY];
                ...
                if ((0, index_2.isNotEmpty)(labelsToCreate)) {
                    try {
                ...
                return await Promise.all(labels.map(async (label) => {
                    ...
                    return await this._commandBus.execute(new commands_1.IntegrationMapSyncLabelCommand({
                        entity: {
                            name,
                            color,
                            description,
                                ...
                                if (!!issueSetting.sync) {
                                    for await (const issue of issues) {
                                        ...
                                        try {
                                            ...
                                            if (!!labelSetting && labelSetting.sync) {
                                                ...
                                                tags = await Promise.all(labels.map(async (label) => {
                                                    ...
                                                    /** */
                                                    return await this._commandBus.execute(new commands_3.AutomationLabelSyncCommand({
                                                        entity: {
                                                            name,
                                                            color,
                                                            description,
                                        ...
                                        try {
                                            ...
                                            return await this._commandBus.execute(new commands_2.AutomationTaskSyncCommand({
                                                entity: {
                                                    title,
                                                    description: body,
                                                    status: state,
...
exports.GithubSyncService = GithubSyncService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [cqrs_1.CommandBus,
        index_1.OctokitService, typeof (_a = typeof integration_tenant_service_1.IntegrationTenantService !== "undefined" && integration_tenant_service_1.IntegrationTenantService) === "function" ? _a : Object, typeof (_b = typeof organization_project_service_1.OrganizationProjectService !== "undefined" && organization_project_service_1.OrganizationProjectService) === "function" ? _b : Object, github_repository_service_1.GithubRepositoryService])
], GithubSyncService);
