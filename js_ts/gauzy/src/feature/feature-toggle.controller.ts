...
import { FeatureEnum, IFeature, IFeatureOrganization, IPagination, PermissionsEnum } from '../../plugins/contracts/dist/index';
...
export class FeatureToggleController {
	...
	async getFeatureToggleDefinitions() {
		...
		if (unleashConfig.url) {
			const enhancedFeatureToggles = getFeatureToggleDefinitions();

			// Transform enhancedFeatureToggles to FeatureInterface[]
			featureToggles = enhancedFeatureToggles.map(toggle => ({
				// map properties accordingly
				...toggle,
				strategies: toggle.strategies.map(strategy => ({
					...strategy,
					segments: strategy.segments.map(segment => segment.id) // or appropriate transformation
				})),
			}));
